...
<div class="demoTable" style="width: 100%;display: block;height: 40px;" >
    <a href="editRole" style="float: right;margin-right: 10px;margin-top: 10px;" class="layui-btn">新增</a>
...
<script>
    layui.use('table', function(){
        ...
        table.on('tool(user)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'edit'){ //查看
               window.location.href ="editRole?roleId="+ data.roleId;
            } else if(layEvent === 'del'){ //删除
                layer.confirm('真的删除行么', function(index){
                     //删除对应行（tr）的DOM结构，并更新缓存
                     layer.close(index);
                     $.ajax({ url: "delRole", data:{roleId:data.roleId}, success: function(data){
                         if(data.code=1){
                             obj.del();
                             layer.confirm('删除成功', function(index){
                                 layer.close(index);
                                 window.location.href="peopleManager";
                             });
                         }
                     }});
                });
