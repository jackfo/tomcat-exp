...
<div class="demoTable" style="width: 100%;display: block;height: 40px;" >
    <a href="addPer" style="float: right;margin-right: 10px;margin-top: 10px;" class="layui-btn">新增</a>
...
<script>
    layui.use('table', function(){
        ...
        table.on('tool(user)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'edit'){ //查看
               window.location.href ="editPer?permissionId="+ data.permissionId;
            } else if(layEvent === 'del'){ //删除
                layer.confirm('真的删除行么', function(index){
                     //删除对应行（tr）的DOM结构，并更新缓存
                     layer.close(index);
                     $.ajax({ url: "delPermission", data:{permissionId:data.permissionId}, success: function(data){
                         if(data.code=1){
                             obj.del();
