...
(function($) {
	...
	function _c(_d) {
		...
		$("span.tree-hit", _f).unbind(".tree").bind("click.tree",
		function() {
			var _10 = $(this).parent();
			_6b(_d, _10[0]);
			return false;
	...
	function _8aa(el) {
		var array = [];
		$(el).find(".tree-checkbox").each(function() {
			var obj = $(this).parent();
			array.push(myChecked(el, obj[0]));
		});
		return array;
	};
	function _8e(_8f) {
	...
	function _36(_ab, _ac) {
		...
		$.data(_ac, "tree-node"), {
			target: _ac,
			checked: $(_ac).find(".tree-checkbox").hasClass("tree-checkbox1")
		});
		if (!_42(_ab, _ac)) {
	 ...
	 * 当子全选或者部分选择时，父均选中
	 */
	function myChecked(_ab, _ac) {
		var _ad = $.extend({},
		$.data(_ac, "tree-node"), {
			target: _ac,
			checked: $(_ac).find(".tree-checkbox").hasClass("tree-checkbox1")||$(_ac).find(".tree-checkbox").hasClass("tree-checkbox2")
		});
		if (!_42(_ab, _ac)) {
			_ad.state = $(_ac).find(".tree-hit").hasClass("tree-expanded") ? "open": "closed";
		}
	...
	$.fn.tree = function(_cb, _cc) {
		...
		return this.each(function() {
			...
			if (_cd) {
				...
				_ce = $.extend({},
				$.fn.tree.defaults, $.fn.tree.parseOptions(this), _cb);
				$.data(this, "tree", {
					options: _ce,
					tree: _1(this)
				});
