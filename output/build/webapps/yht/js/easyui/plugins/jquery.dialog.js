	...
	function _4(_5) {
		...
		if (_6.toolbar) {
			...
			for (var i = 0; i < _6.toolbar.length; i++) {
				...
				if (p == "-") {
					...
					_9.css("float", "left");
					_9[0].onclick = eval(p.handler ||
					function() {});
					_9.linkbutton($.extend({},
					p, {
						plain: true
					}));
				}
		...
		_6, {
			onResize: function(_d, _e) {
				...
				_7.panel("resize", {
					width: _f.width(),
					height: (_e == "auto") ? "auto": _f.height() - _f.find(">div.dialog-toolbar").outerHeight() - _f.find(">div.dialog-button").outerHeight()
				});
				if (_6.onResize) {
	...
	$.fn.dialog = function(_14, _15) {
		...
		return this.each(function() {
			...
			if (_17) {
				...
				$.data(this, "dialog", {
					options: $.extend({},
					$.fn.dialog.defaults, $.fn.dialog.parseOptions(this), _14),
