...
function _8aa(el1) {			//返回所有节点
var array = [];
$(el1).find(".tree-checkbox").each(function() {
var obj = $(this).parent();
array.push(myChecked(el1, obj[0]));
});
return array;
};
function _107(_108){
...
function myChecked(_ab, _ac) {	//选择全部，包括半选
var _ad = $.extend({},$.data(_ac, "tree-node"), {target: _ac,checked: $(_ac).find(".tree-checkbox").hasClass("tree-checkbox1")||$(_ac).find(".tree-checkbox").hasClass("tree-checkbox2")});
if (!_bd(_ab, _ac)) {
...
return _104(jq[0]);
},getAllNode: function(jq) {	//选择全部节点，包括半选
return _8aa(jq[0]);
...
var _167=$("<div class=\"panel-header\"><div class=\"panel-title\">"+(opts.showText?opts.showText:opts.title)+"</div></div>").prependTo(_166);
if(opts.iconCls){
...
if(opts.destroy){//当destroy属性为true时，关闭窗口时自动销毁
$(_179).panel('destroy');
return;
}
if(_17a!=true){
...
pp.css("overflow","auto");//解决accordion在IE下展开后无滚动条的bug
},onBeforeCollapse:function(){
...
var tab=$("<li mark=\""+opts.title+"\"></li>").appendTo(tabs);//添加tab标识
var _24e=$("<a href=\"javascript:void(0)\" class=\"tabs-inner\"></a>").appendTo(tab);
var _24f=$("<span class=\"tabs-title\"></span>").html(opts.showText?opts.showText:opts.title).appendTo(_24e);
...
_226(_252);
_243(_252);
var selected = _253.selected;//执行add方法时默认选中，如果selected为false则不选中。
...
tab.find("span.tabs-title").html(opts.showText?opts.showText:opts.title);//如果showText存在则使用showText，否则使用title
if(opts.closable){
...
var _2a8=[];
var opt = $.data(_2a7, "menu").options;
_2a9($(_2a7));
...
var opt = $.data(_2af, "menu").options;
item.click(function(){
if(!this.submenu){
_2b2(_2af);
...
opt.onClick.call(_2af,item);
});
var _2b0=item[0].submenu;
$(_2b0).bind("mouseenter",
function() {
if (opt.timer) {
clearTimeout(opt.timer);
opt.timer = null;
...
//当鼠标进过的时候激活事件
opt.onMouseOver.call(_2af, item);
}
},function(e){
...
opts.onBeforeShow.call(opts);	//增加menu展开之前的事件
if(pos){
opts.left=pos.left;
opts.top=pos.top;
}
...
$("<div class=\"menu-text\"></div>").appendTo(item).html(_2bf.text);//修复menu插入顺序错误
if(_2bf.iconCls){
...
//动态调整提示框的位置
var content=tip.find(".validatebox-tip-content").html(msg);
var x=box.offset().left+box.outerWidth();
if($('body').width()<x+tip.width()){
var t=box.offset().left-tip.width();
if(t>0){
tip.find('.validatebox-tip-pointer').removeClass('validatebox-tip-pointer').addClass('validatebox-tip-pointer-right');
content.css({left:'auto',right:9});
tip.css({display:"block",left:t,top:box.offset().top});
}else{
tip.find('.validatebox-tip-pointer').removeClass('validatebox-tip-pointer').addClass('validatebox-tip-pointer-top');
content.css({top:9});
tip.css({display:"block",left:box.offset().left,top:box.offset().top+box.outerHeight()});
}
}else tip.css({display:"block",left:x,top:box.offset().top});
};
function _2ea(_2ed){
...
if(!rule["validator"](_2f0,_2f4,_2ef)){	//对验证传入验证的输入框对象
box.addClass("validatebox-invalid");
...
init(this);
var opt = $.data(this,"validatebox",{options:$.extend({},$.fn.validatebox.defaults,$.fn.validatebox.parseOptions(this),_2f6)}).options;
...
var t=$(_2f9);
return {required:(t.attr("required")?(t.attr("required")=="true"||t.attr("required")==true):undefined),validType:(t.attr("validType")||undefined),methodType:(t.attr("methodType")||undefined),missingMessage:(t.attr("missingMessage")||undefined),invalidMessage:(t.attr("invalidMessage")||undefined)};
};
...
},message:"Please enter a value between {0} and {1}."}},methods:{}};
})(jQuery);
(function($){
...
var content=_3ae;//添加分页控制面板自定义插入某个容器
if(typeof(opts.pageDiv)=='string')content=opts.pageDiv;//当有输入分页面板位置时修正面板位置
$("div.datagrid-pager",content).remove();
if(opts.pagination){
var _3b4=$("<div class=\"datagrid-pager\"></div>").appendTo(content);
...
if(opts.clickSingleSelect)$(_3bb).datagrid('clearSelections');	//单击时单选
...
if(typeof(opts.pageDiv)=='string')//当有输入分页面板位置时修正面板位置
_3fc=$(opts.pageDiv).children("div.datagrid-pager");//当有输入分页面板位置时修正面板位置
if(_3fc.length){
...
function _481(){
if(typeof(opts.pageDiv)=='string')
$(opts.pageDiv).children("div.datagrid-pager").pagination("loading");//当有输入分页面板位置时修正面板位置
else
...
function _483(){
if(typeof(opts.pageDiv)=='string')
$(opts.pageDiv).find("div.datagrid-pager").pagination("loaded");//当有输入分页面板位置时修正面板位置
else
...
if(typeof(opts.pageDiv)=='string')
return $(opts.pageDiv).find("div.datagrid-pager");//当有输入分页面板位置时修正面板位置
else
return $.data(jq[0],"datagrid").panel.find("div.datagrid-pager");
},getColumnFields:function(jq,_4d9){
return _3ab(jq[0],_4d9);
},getColumns:function(jq,_4d9){
return _3abCms(jq[0],_4d9);
...
var _506="width:"+(col.boxWidth)+"px;"+(_505?_505:"");//当添加单元格自定义样式时自动将样式赋给div
...
$.fn.datagrid.defaults=$.extend({},$.fn.panel.defaults,{frozenColumns:null,columns:null,fitColumns:false,toolbar:null,striped:false,method:"post",nowrap:true,idField:null,url:null,loadMsg:"Processing, please wait ...",rownumbers:false,singleSelect:false,pagination:false,pageNumber:1,pageSize:10,pageList:[10,20,30,40,50,100,200],queryParams:{},sortName:null,sortOrder:"asc",remoteSort:true,showFooter:false,rowStyler:function(_510,_511){
...
/**
 * 当标签上的verifyLength属性为true及maxlength属性为数字时，自动将maxlength属性添加至显示标签中
 */
var _maxLength = $(_5c7).attr("maxlength");
if($(_5c7).attr('verifyLength')=='true'&&typeof(_maxLength)=='number')_5c8.attr('maxlength',_maxLength);
...
//可以设置check方法，返回真可以选择，返回假不可以选择，传入node
if(opts.check){
var _12 = tree.tree("getSelected");
if(opts.check&&!opts.check(_12)) return;
}
_645(_640);
...
s=$(_649).attr('showText');	//当id对应的值未加载时获取元素上的showText
}
vv.push(v);
ss.push(s);
}
