...
<html>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				table = $('#dataTable').datagrid({
					url:'${path}/hospital/vaccine!list.qt?random='+getRandomNum(),
					...
					columns:[[
						...
						{field:'name',width:200,title:'疫苗名称'},
						{field:'descContent',width:300,title:'疫苗描述'},
						{field:'addTime',width:100,title:'录入时间'}
					]],
	...
	<body>
		<div class="buttonPanel">
			<div class="formPanel">
				<label for="vaccineType.name">疫苗名称：</label>
				<input type="text" name="vaccineType.name"  style="float:left;"/>
				<div class="clear"></div>
			</div>
			<fs:button key="NEWADD" id="addButton" name="新增" iconCls="icon-add">
				function(){
					win.show('新增疫苗类型','${path}/hospital/vaccine!preAdd.qt?chkAccess='+encodeURIComponent('{#chkAccess}')+'&random='+getRandomNum(),table);
					win.reSize(500,200);
				}
			</fs:button>
			<fs:button key="MODIFY" id="modifyButton" name="修改" iconCls="icon-edit">
				function(){
					...
					win.show('修改疫苗类型','${path}/hospital/vaccine!preModify.qt?id='+rows[0].id+'&chkAccess='+encodeURIComponent('{#chkAccess}')+'&random='+getRandomNum(),table);
					win.reSize(500,200);
				}
			</fs:button>
						...
						parent.jQuery.messager.confirm('提示','确定删除'+names+'等记录吗？',function(b){
							if(b){
								jQuery.ajax({
									async: false,
									type: "GET",
									url: '${path}/hospital/vaccine!delete.qt?id='+encodeURIComponent(ids)+'&chkAccess='+encodeURIComponent('{#chkAccess}')+'&random='+getRandomNum(),
			...
			<fs:button key="QUERY" id="queryBtn" name="查询" iconCls="icon-search">
				function(){
					...
					table.datagrid('options').url='${path}/hospital/vaccine!list.qt?chkAccess='+encodeURIComponent('{#chkAccess}')+'random='+getRandomNum()+params;
					table.datagrid('reload');
					table.datagrid('clearSelections');
