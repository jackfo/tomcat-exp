...
<title>修改</title>
	...
	<script type="text/javascript">
		...
		function submitForm(el){
			$('#newForm').form('submit',{
				url:'${path}/db/conn!modify.qt?random='+getRandomNum()+'&chkAccess='+encodeURIComponent('${param.chkAccess}'),
				onSubmit:function(){
	...
	<body>
		<form id="newForm" name="newForm" method="post">
			...
			<input type="hidden" name="connection.id" value="${item.id }"/>
			<input type="hidden" id="connection.clazz" name="connection.clazz" value="${item.clazz }">
			<input type="hidden" id="connection.url" name="connection.url" value="${item.url }">
			<input type="hidden" id="connection.driver.id" name="connection.driver.id" value="<c:catch>${item.driver.id }</c:catch>">
			<table style="width:100%;table-layout:fixed;" border="0" cellpadding="0" cellspacing="0">
	    		<tr>
					<th>名称：</th>
					<td>
						<input type="text" id="connection.name" name="connection.name" value="${item.name }" class="easyui-validatebox" validType="exist['${path }/db/conn!isExist.qt?chkAccess=${param.chkAccess}','${item.name }','^[.\\-\\w\\u4E00-\\u9FA5]+$']" required="true" maxlength="50" style="width:70%;"/>
					</td>
				</tr>
				<tr>
					<th>数据库驱动：</th>
	    			<td>
						<select id="diver" style="width:70%;"><c:forEach var="driver" items="${drivers }">
							<option value="['${driver.id }','${driver.clazz }','${driver.url }']"<c:catch><c:if test="${driver.id eq item.driver.id }"> selected="selected"</c:if></c:catch>>${driver.name }</option></c:forEach>
						</select>
	    			</td>
	    		</tr>
	    		<tr>
	    			<th>服务器地址[IP]：</th>
	    			<td>
	    				<input type="text" id="ip" style="width:70%;" value="${ip }"/>
	    			</td>
	    		</tr>
	    		<tr>
	    			<th>端口[PORT]：</th>
	    			<td>
	    				<input type="text" id="port" style="width:70%;" value="${port }"/>
	    			</td>
	    		</tr>
	    		<tr>
	    			<th>数据库名[DB]：</th>
	    			<td>
	    				<input type="text" id="db" style="width:70%;" value="${db }"/>
	    			</td>
	    		</tr>
	    		<tr>
					<th>用户名：</th>
	    			<td>
	    				<input type="text" id="connection.user" value="${item.user }" name="connection.user" class="easyui-validatebox" maxlength="100" style="width:70%;"/>
	    			</td>
	    		</tr>
	    		<tr>
					<th>密码：</th>
	    			<td>
	    				<input type="text" id="connection.pass" value="${item.pass }" name="connection.pass" class="easyui-validatebox" maxlength="100" style="width:70%;"/>
	    			</td>
	    		</tr>
	    		<tr>
