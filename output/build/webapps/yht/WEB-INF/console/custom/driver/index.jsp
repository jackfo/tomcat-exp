...
<!--
	...
	$(document).ready(function(){
		...
		table=$("#dataList").datagrid({
			...
			striped: true,
			collapsible:false,
			url:'${path}/db/driver!list.qt?random='+getRandomNum()+'&chkAccess='+encodeURIComponent('${param.chkAccess }'),
			border:false,
			pagination:true,
			rownumbers:true,
			remoteSort: false,
...
<body>
	<div class="buttonPanel">
		<div class="formPanel">
			<label for="driver.name">名称：</label>
			<input type="text" name="driver.name" id="driver.name" style="float:left;"/>
			<label for="driver.clazz">Class：</label>
			<input type="text" name="driver.clazz" id="driver.clazz" style="float:left;"/>
			<label for="driver.url">URL：</label>
			<input type="text" name="driver.url" id="driver.url" style="float:left;"/>
			<div class="clear"></div>
		</div>
		<div class="datagrid-btn-separator" style="visibility:hidden;"></div>
		<fs:button key="NEWADD" id="addBtn" name="新增" iconCls="icon-add">
			function(){
				win.show('新增','${path}/db/driver!preAdd.qt?random='+getRandomNum()+'&chkAccess='+encodeURIComponent('{#chkAccess}'),table);
				win.reSize(600,300);
			}
		</fs:button>
		<fs:button key="MODIFY" id="modifyBtn" name="修改" iconCls="icon-edit">
			function(){
				...
				win.show('修改','${path}/db/driver!preModify.qt?id='+rows[0].id+'&chkAccess='+encodeURIComponent('{#chkAccess}')+'&random='+getRandomNum(),table);
				win.reSize(600,300);
			}
		</fs:button>
					...
					if(ids == ''){
						...
						if(b){
							jQuery.ajax({
								async: false,
								type: "GET",
								url: '${path}/db/driver!delete.qt?id='+encodeURIComponent(ids)+'&chkAccess='+encodeURIComponent('{#chkAccess}')+'&random='+getRandomNum(),
		...
		<fs:button key="QUERY" id="queryBtn" name="查询" iconCls="icon-search">
			function(){
				if(!$(this).linkbutton("options").disabled){
					...
					table.datagrid('options').url='${path}/db/driver!list.qt?random='+getRandomNum();
