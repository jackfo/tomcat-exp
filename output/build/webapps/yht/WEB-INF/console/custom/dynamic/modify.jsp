...
<title>修改</title>
	...
	<script type="text/javascript">
		function submitForm(el){
			$('#newForm').form('submit',{
				url:'${path}/custom/dynamic!modify.qt?chkAccess=${param.chkAccess}&random='+getRandomNum(),
				onSubmit:function(){
				...
				<c:catch><c:if test="${not empty form}"><c:forEach var="item" items="${form.attrs}"><tr>
					...
					<td>
						<input type="text" id="dt.attrValues.${item.name }" name="dt.attrValues" maxlength="${item.length }" style="width:150px" class="easyui-validatebox"<c:if test="${item.clazz eq 'java.sql.Timestamp' }">
							 methodType="chooseDate{dateFmt:'yyyy-MM-dd HH:mm:ss'}" value="<fmt:formatDate value="${dataMap[item.name] }" pattern="yyyy-MM-dd HH:mm:ss"/>"</c:if><c:if test="${item.clazz eq 'java.sql.Date' }">
							 methodType="chooseDate" value="<fmt:formatDate value="${dataMap[item.name] }" pattern="yyyy-MM-dd"/>"</c:if><c:if test="${item.clazz eq 'java.sql.Time' }">
							 methodType="chooseDate{dateFmt:'HH:mm:ss'}" value="<fmt:formatDate value="${dataMap[item.name] }" pattern="HH:mm:ss"/>"</c:if><c:if test="${item.clazz eq 'java.lang.Long' or item.clazz eq 'java.lang.Integer' }">
							 validType="decimals['只能输入整数！','^[\\-]?[\\d]+$']" value="${dataMap[item.name] }"</c:if><c:if test="${item.clazz eq 'java.lang.Float' or item.clazz eq 'java.lang.Double' }">
							 validType="decimals['只能输入小数','^[\\d]+(\\.[\\d]+)?$']" value="${dataMap[item.name] }"</c:if><c:if test="${item.clazz eq 'java.lang.String' }">
							 value="${dataMap[item.name] }"</c:if><c:if test="${item.required eq 1 }">
							 required="true"</c:if><c:if test="${item.unique eq 1 }">
							 validType="exist['${path }/custom/dynamic!isExist.qt?entityName=${form.name }&key=${item.name }&clazz=${item.clazz }&chkAccess=${param.chkAccess}','${dataMap[item.name] }']"</c:if><c:if test="${item.mobile eq 1 }">
