...
<html>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				table = $('#dataTable').datagrid({
					url:'${path}/source/mmMusicTypeBean!list.qt?random='+getRandomNum(),
					...
					columns:[[
						...
						{field:'musicTypeName',width:200,title:'音乐类型名称'},
	...
	<body>
		<div class="buttonPanel">
			<div class="formPanel">
				<label for="mmMusicTypeBean.musicTypeName" style="width: 100">音乐类别名称：</label>
				<input type="text" name="mmMusicTypeBean.musicTypeName" id="mmMusicTypeBean.musicTypeName" style="float:left;"/>
				<div class="clear"></div>
			</div>
			<fs:button key="NEWADD" id="addButton" name="新增" iconCls="icon-add">
				function(){
					win.show('新增音乐类型','${path}/source/mmMusicTypeBean!preAdd.qt?chkAccess='+encodeURIComponent('{#chkAccess}')+'&random='+getRandomNum(),table);
					win.reSize(500,200);
				}
			</fs:button>
			<fs:button key="MODIFY" id="modifyButton" name="修改" iconCls="icon-edit">
				function(){
					...
					win.show('修改音乐类型','${path}/source/mmMusicTypeBean!preModify.qt?mmMusicTypeBean.id='+rows[0].id+'&chkAccess='+encodeURIComponent('{#chkAccess}')+'&random='+getRandomNum(),table);
					win.reSize(500,200);
				}
			</fs:button>
			<fs:button key="DELETE" id="deleteButton" name="删除" iconCls="icon-remove">
				function(){
					...
					if(rows && 1!=rows.length) parent.jQuery.messager.alert('错误','对不起！请选择将要删除的一条记录！','error');
						...
						jQuery.messager.confirm('提示','确定删除图片类型: \"'+rows[0].musicTypeName+'\" 吗？',function(b){
							if(b){
								jQuery.ajax({
									async: false,
									type: "GET",
									url: '${path}/source/mmMusicTypeBean!delete.qt?mmMusicTypeBean.id='+rows[0].id+'&chkAccess='+encodeURIComponent('{#chkAccess}')+'&random='+getRandomNum(),
			...
			<fs:button key="QUERY" id="queryBtn" name="查询" iconCls="icon-search">
				function(){
					...
					table.datagrid('options').url='${path}/source/mmMusicTypeBean!list.qt?chkAccess='+encodeURIComponent('{#chkAccess}')+'random='+getRandomNum()+params;
					table.datagrid('reload');
					table.datagrid('clearSelections');
