	...
	<head>
		...
		<script type="text/javascript">
			...
			function query(){
				...
				url = "${path}/sms/chart!chartForReceived.qt?chkAccess="+encodeURIComponent('${fs:chkAccess(param.mid,'CHART')}')+getURLParams();</c:if><c:if test="${param.type eq 'sent'}">
				url = "${path}/sms/chart!chartForSent.qt?chkAccess="+encodeURIComponent('${fs:chkAccess(param.mid,'CHART')}')+getURLParams();</c:if>
			...
			<div class="formPanel">
				<table>
					<tr>
						...
						<td>
							<input type="text" name="year" class="easyui-validatebox" value="<%=new SimpleDateFormat("yyyy").format(new Date()) %>" methodType="chooseDate{dateFmt:'yyyy'}" onFocus="WdatePicker({dateFmt:'yyyy',readOnly:true})"/>
						</td>
						<td>
						...
						<td>
							<fs:button key="RETOTAL" id="RETOTAL_BTN" name="重新汇总" iconCls="icon-search">
								function(){
									win.show('重新汇总','${path}/sms/chart!total.qt?chkAccess='+encodeURIComponent('{#chkAccess}')+'&random='+getRandomNum());
									win.reSize(300,150);
								}
							</fs:button>
						</td></c:if>
					...
					<tr>
						<td colspan="5" style="padding-left: 20px;color: red;">注意：如果查询年度没有数据，则不能显示饼状图！</td>
					</tr>
				</table>
				<div class="clear"></div>
			</div>
