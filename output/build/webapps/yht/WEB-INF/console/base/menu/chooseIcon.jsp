	...
	<head>
		...
		<style type="text/css">
			*{margin: 0px;padding: 0px;overflow: hidden;font-size: 12px;background-color: white;}
			...
			.iconContent .iconItem{float: left;margin: 0px;padding: 0px;overflow: hidden;}
		</style>
	...
	<body>
		<div class="iconContent">
			<c:forEach begin="1" end="520" step="1" varStatus="s">
				<div onclick="submitForm(this)" title="点击选择并返回" value="icon-${s.index }" class="iconItem iconMain icon-${s.index }"></div>
				<c:if test="${s.index % 30 == 0}"><div style="clear: both;"></div></c:if>
			</c:forEach>	
		</div>
	</body>
</html>