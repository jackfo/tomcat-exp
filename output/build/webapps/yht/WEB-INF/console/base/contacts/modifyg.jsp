...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function submitForm(el){
				$('#newForm').form('submit',{
					url:'${path}/base/contacts!gmodify.qt?random='+getRandomNum(),
					onSubmit:function(){
					...
					this.css({color:'#7d7d7d'}).focus(function(){
						...
						if(str!=''){
							jQuery.ajax({
								//async:false,
								type:"POST",
								url:'${path}/base/contacts!preGmodify.qt?s='+encodeURIComponent(str)+'&chkAccess='+encodeURIComponent('${param.chkAccess}')+'&random='+getRandomNum(),
								dataType:"json",
								success:function(data){
	...
	<body>
		<form id="newForm" name="newForm" method="post">
			...
			<input type="hidden" name="group.parentId" value="${group.parentId }">
			<input type="hidden" name="group.id" value="${group.id }">
			<table style="width:100%;table-layout:fixed;" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<th>组名：</th>
					<td>
						<input type="text" id="group.name" name="group.name" value="${group.name }" class="easyui-validatebox" required="true" style="width:98%" maxlength="50"/>
					</td>
					<th>编号：</th>
					<td>
						<input type="text" id="group.code" name="group.code" value="${group.code }" class="easyui-validatebox" required="true" style="width:100%" maxlength="3" disabled="disabled"/>
					</td>
					<td style="width:50px;" align="right"><c:if test="${fs:isAccess(param.mid,'SHARE') }">
						<input type="checkbox" id="group.type" name="group.type" value="1"<c:if test="${group.type eq 1 }"> checked="checked"</c:if>/>
				...
				<tr>
					...
					<td colspan="4" style="height:1%">
						<div id="fastSearchSelectBox" style="width:100%;height:135px;*height:137px;border:1px solid #7F9DB9;background:#fff;overflow:auto;overflow-x:hidden;"><c:forEach var="item" items="${pageList.items }">
							<div class="g-selectBox-item g-selectBox-checked" s_id="${item.id }">
								<input type="checkbox" name="conId" value="${item.id }" checked="checked">
				...
				<tr>
					...
					<td colspan="4" style="height:1%;padding:5px 0">
						<div id="fastSearchTargetBox" style="min-height:22px;height:1%\9;padding:0px 3px 3px 0px;border:1px solid #7F9DB9;background:#fff;color:#7d7d7d;">
