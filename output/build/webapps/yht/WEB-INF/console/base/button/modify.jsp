...
<html>
	<head>
		...
		<script type="text/javascript">
			function submitForm(el){
				$('#newForm').form('submit',{
					url:'${path}/base/menu!btnModify.qt?random='+getRandomNum(),
					onSubmit:function(){
	...
	<body>
		<form id="newForm" name="newForm" method="post">
			...
			<input type="hidden" name="button.id" value="${item.id }"/>
			<table style="width:100%;table-layout:fixed;" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<th>所属菜单：</th>
					<td><c:catch><c:out value="${item.menu.menuName}"/></c:catch></td>
				</tr>
				<tr>
					<th>按钮名称：</th>
	    			<td>
	    				<input type="text" id="button.buttonName" name="button.buttonName" value="${item.buttonName }" class="easyui-validatebox" required="true" maxlength="50" style="width:200px"/>
	    			</td>
	    		</tr>
	    		<tr>
	    			<th>按钮标识：</th>
	    			<td>
	    				<input type="text" id="button.buttonMark" name="button.buttonMark" value="${item.buttonMark }" style="width:200px" maxlength="15" disabled="disabled"/>
	    			</td>
	    		</tr>
	    		<tr>
	    			<th style="height:70px;" valign="top">访问资源：</th>
	    			<td>
	    				<textarea id="button.accessRes" name="button.accessRes" rows="4" style="width:200px" class="easyui-validatebox" validType="length[0,1000]"><c:out value="${item.accessRes}"/></textarea>
	    			</td>
	    		</tr>
	    		<tr>
