...
<style type="text/css">
	...
	<script type="text/javascript">
		...
		function submitForm(el){
			$('#newForm').form('submit',{
				url:'${path}/base/timer!newAdd.qt?chkAccess=${param.chkAccess}&random='+getRandomNum(),
				onSubmit:function(){
		...
		<form id="newForm" name="newForm" method="post">
			...
			<table style="width:100%;table-layout:fixed;" border="0" cellpadding="0" cellspacing="0">
	    		<tr>
	    			<th>定时器名称：</th>
	    			<td>
	    				<input id="timerData.name" name="timerData.name" class="easyui-validatebox" required="true" style="width:70%;">
	    			</td>
	    		</tr>
				<tr>
					<th>定时器类型：</th>
					<td><select id="type" name="timerData.type" onchange="setDateFmt()" style="width:70%;"><option value="1">定点执行</option><option value="2">定时轮询</option></select></td>
				</tr>
				<tr>
	    			<th>格式类型：</th>
	    			<td>
	    				<div id="type1" style="display: block;">
	    					<input id="dateFmt" name="timerData.cronFmt" value="HH:mm" onchange="setDateFmt()" class="easyui-validatebox" required="true" style="width:70%">
	    				</div>
	    				<div id="type2" style="display: none;">
		    				<input type="radio" name="jgFmt" value="ss" checked="checked" onclick="setDateFmt()"/>秒
		    				<input type="radio" name="jgFmt" value="mm" onclick="setDateFmt()"/>分
		    				<input type="radio" name="jgFmt" value="HH" onclick="setDateFmt()"/>时
		    				<input type="radio" name="jgFmt" value="dd" onclick="setDateFmt()"/>天
	    				</div>
	    			</td>
	    		</tr>
				<tr>
	    			<th>时间设置：</th>
	    			<td>
	    				<input id="timeSet" name="timerData.cronTime" class="easyui-validatebox" required="true" methodType="chooseDate{dateFmt:'yyyy-MM-dd HH:mm:ss'}" style="width:70%">
	    			</td>
	    		</tr>
	    		<tr>
	    			<th>定时器任务：</th>
	    			<td>
	    				<input id="timerData.service" name="timerData.service" style="width:70%">
	    			</td>
	    		</tr>
	    		<tr>
	    			<th>状态：</th>
	    			<td>
						<input type="radio" id="status1" name="timerData.status" value="1" checked="checked">
						<label for="status1">启用</label>
	    				<input type="radio" id="status" name="timerData.status" value="0">
				...
				<tr>
	    			...
	    			<td style="height: 100px;">
	    				<textarea name="timerData.describe" rows="6" cols="" style="width:70%;"></textarea>
	    			</td>
	    		</tr>
	    		<tr>
