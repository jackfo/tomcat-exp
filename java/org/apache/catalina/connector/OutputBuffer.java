...
public class OutputBuffer extends Writer
    ...
    public void close()
        ...
        //将缓冲去的字符刷新给页面，这里面的数据来自servlet中 response.getWrite()
        if (cb.getLength() > 0) {
            cb.flushBuffer();
        }
    ...
    protected void doFlush(boolean realFlush) throws IOException {
        ...
        try {
            doFlush = true;
            if (initial) {
                coyoteResponse.sendHeaders();//发送头部数据
    ...
    public void realWriteBytes(byte buf[], int off, int cnt)
        ...
        if (cnt > 0) {
            ...
            try {
                //tomcat的response来写数据，其实是写到httpprocessor的buffer里面去了
