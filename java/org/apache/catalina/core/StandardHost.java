...
public class StandardHost extends ContainerBase implements Host {
     ...
     */
    private boolean copyXML = false;  //sDeployXML方法指明host是否要部署一个描述文件，默认为false
    ...
    public void setContextClass(String contextClass) {
        ...
        //将一个事件发送给所有已经注册的监听器。该方法有三个参数：属性的名字、旧的值以及新的值。属性的值必须是对象，如果是简单数据类型，则必须进行包装
    ...
    /**
     * 添加一个子容器即Context 调用deployDescriptor部署描述符是
     * 在添加到之前会给这个子容器设置一个生命周期监听器MemoryLeakTrackingListener实例
     * 然后调用父类的addChild最终调用ContainBase中addChildInternal方法添加到对应集合
     * */
    @Override
    ...
    /**只有StandContext在after_start才会真的调用这个方法*/
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        if ((errorValve != null) && (!errorValve.equals(""))) {
            try {
                ...
                //将errorValve==org.apache.catalina.valves.ErrorReportValve给添加进去
