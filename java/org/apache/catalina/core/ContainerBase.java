...
public abstract class ContainerBase extends LifecycleMBeanBase
    ...
    public static final String module = ContainerBase.class.getName();
    ...
    private void addChildInternal(Container child) {
        Debug.log("Add child " + child + " " + this,module);
        ...
        try {
            ...
            Debug.logError(e,"ContainerBase.addChild: start: ",module);
     ...
     * @author 郑小康
     *
     * 1.如果配置了集群组件Cluster则启动
     *
     * 2.如果配置了安全组件,则启动
     *
     * 3启动子节点，默认为StandContext
     *
     * 4.启动Host所持有的Pipeline组件
     *
     * 5.设置Host状态为STARTING 此时会触发START_EVENT生命周期事件
     *   HostConfig 中的lifecycleEvent 为START_EVENT时会调用其start方法
     *   HostConfig监听该事件 扫描web部署 对于部署文件、WAR包、会自动创建StandardContext实例，添加到Host并启动
     *
     * 6.启动Host层级的后台任务处理 包括部署变更
     *
     * */
    @Override
     ...
     * 2.获取用户管理（不是我研究的重点）
     *
     * 3.执行其阀门下面所有的backgroundProcess方法，可以看出这是一个递归执行backgroundProcess
     *
     *   engine  -->   host  ---->  context  这是一个大致过程
     *
     * 4.调用生命周期的监听方法，修改状态为PERIODIC_EVENT
     * */
    @Override
     ...
     *
     *    StandardEngine，StandardHost都继承了当前类，是否都启动了这个线程
     *
     *    其实不然，这就是backgroundProcessorDelay的作用，在StandardEngine实例化的时候其赋值为10
     *    而StandardHost却并没有，所以只有在StandardEngine调用startInternal的时候才会启动线程
     */
    ...
    /**
     * 在run方法中它会先暂停一段时间之后调用processChildren方法
     * */
    protected class ContainerBackgroundProcessor implements Runnable {
        ...
        /**
         * processChildren方法做了两件事，
         * 一是调用容器组件自身的backgroundProcess方法，二是取出该容器组件的所有子容器组件并调用它们的processChildren方法。
         * 归结起来这个线程的实现就是定期通过递归的方式调用当前容器及其所有子容器的backgroundProcess方法。
