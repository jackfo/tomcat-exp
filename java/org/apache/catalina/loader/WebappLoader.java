...
public class WebappLoader extends LifecycleMBeanBase
    ...
    protected void startInternal() throws LifecycleException {
        ...
        Debug.log(sm.getString("webappLoader.starting"),module);

        if (context.getResources() == null) {
            Debug.log("No resources for " + context,module);
        ...
        try {
            //创建对应的web应用加载器 WebappClassLoaderBase注入StandardContext中的parent这个ClassLoader句柄
            classLoader = createClassLoader();
            //获取对应的资源 即StandardRoot实例将其注入到classLoader
            ...
            Debug.logError(t,"LifecycleException ",module);
    ...
    private WebappClassLoaderBase createClassLoader()
        throws Exception {
        //获取org.apache.catalina.loader.WebappClassLoader
        ...
        //StandardContext的类加载器 参数是ClassLoader 原因:public WebappClassLoader(ClassLoader parent)
        Class<?>[] argTypes = { ClassLoader.class };
        Object[] args = { parentClassLoader };
        Constructor<?> constr = clazz.getConstructor(argTypes);
        classLoader = (WebappClassLoaderBase) constr.newInstance(args);
        //返回对应的WebappClassLoader
        return classLoader;
    }
    ...
    /**
     *注入class path到context的属性,Jasper的编译必须依赖于它
     */
    private void setClassPath() {
        //验证当前Context是否存在
        if (context == null)
            return;
        //获取对应的ApplicationContextFacade实例
        ServletContext servletContext = context.getServletContext();
        ...
        //如果是双亲委托机制,在上面就不会web加载器的环境 所以在这里需要添加一下
        if (delegate) {
        ...
        //储存这个classpath到org.apache.catalina.jsp_classpath
        servletContext.setAttribute(Globals.CLASS_PATH_ATTR, this.classpath);
    }


    /**
     * 获取当前加载器的所有url加载到classpath中去
     * */
