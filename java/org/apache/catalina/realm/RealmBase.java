...
public abstract class RealmBase extends LifecycleMBeanBase implements Realm {
    ...
    public SecurityConstraint [] findSecurityConstraints(Request request, Context context) {
        ...
        //找到当前context下所有约束
        //获取web.xml中所有根据security-constraint解析的实例
        SecurityConstraint constraints[] = context.findConstraints();
        if ((constraints == null) || (constraints.length == 0)) {
            Debug.log(" No applicable constraints defined",module);
            return (null);
        }
        ...
        boolean found = false;
        /**
         * 找到web-resource-collection标签所构成的实例
         * */
        for (i = 0; i < constraints.length; i++) {
            ...
            Debug.log("  Checking constraint '" + constraints[i] + "' against " + method + " " + uri + " --> " + constraints[i].included(uri, method),module);
            //找到对应匹配规则
        ...
        if(results == null) {
            ...
            Debug.log("No applicable constraint located",module);
        }
        return resultsToArray(results);
    }
    ...
    public boolean hasUserDataPermission(Request request, Response response, SecurityConstraint []constraints) throws IOException {
        ...
        if (constraints == null || constraints.length == 0) {
            Debug.log("  No applicable security constraint defined",module);
            return (true);
        }
        //遍历所有约束
        for(int i=0; i < constraints.length; i++) {
            ...
            if (userConstraint == null) {
                Debug.log("No applicable user data constraint defined",module);
                return (true);
            }
            //用于数据没有限制
            if (userConstraint.equals(Constants.NONE_TRANSPORT)) {
                Debug.log("User data constraint has no restrictions",module);
                return (true);
            }
        ...
        if (request.getRequest().isSecure()) {
            Debug.log("User data constraint already satisfied",module);
            return (true);
        }
        ...
        if (redirectPort <= 0) {
            Debug.log("SSL redirect is disabled");
            //403
        ...
        Debug.log("Redirecting to " + file.toString(),module);
