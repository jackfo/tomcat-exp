...
public final class ClassLoaderFactory {
    ...
    /**
     * @author 郑小康
     * 每当需要装载一个类的时候，会首先调用系统类装载器。
     * 但是系统类装载器并不会立即装载，而是将其交给父装载器：扩展类装载器。
     * 扩展类装载器其将交给引导类装载器。
     * 引导类装载器没有父装载器，它会尝试装载这个类，如果找不到这个类，会交给扩展类装载。
     * 如果扩展类装载器还是没有找到，会交给系统类装载器。
     * 如果系统类装载器还是没有找到这个类，则会抛出java.lang.ClassNotFoundException异常。
     * 代理模式主要是为了解决类装载的安全问题。例如：对于自定类的java.lang.Object类，永远得不到装载，除非，rt.jar中确实没有这个类。
     * */
