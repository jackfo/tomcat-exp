...
public abstract class AuthenticatorBase extends ValveBase
    ...
    public void invoke(Request request, Response response)
        ...
        Debug.log("安全检查请求" + request.getMethod() + " " + request.getRequestURI(),module);
        ...
        if (cache) {
            ...
            if (principal == null) {
                ...
                if (session != null) {
                    ...
                    if (principal != null) {
                        Debug.log("We have cached auth type " + session.getAuthType() + " for principal " + session.getPrincipal(),module);
        ...
        //如果URI以contextpath开始并以j_security_check结尾
        if (decodedRequestURI.startsWith(contextPath) && decodedRequestURI.endsWith(Constants.FORM_ACTION)) {
            if (!authenticate(request, response)) {
                Debug.log(" Failed authenticate() test ??" + decodedRequestURI ,module);
                return;
            }
        }
        ...
        Session session = request.getSessionInternal(false);
        if (session != null) {
            //获取org.apache.catalina.authenticator.REQUEST中的属性
            SavedRequest savedRequest = (SavedRequest) session.getNote(Constants.FORM_REQUEST_NOTE);
            if (savedRequest != null && decodedRequestURI.equals(savedRequest.getDecodedRequestURI()) && !authenticate(request, response)) {
                Debug.log("Failed authenticate() test",module);
                return;
            }
        }
        ...
        if (constraints != null && disableProxyCaching && !"POST".equalsIgnoreCase(request.getMethod())) {
            if (securePagesWithPragma) {
                ...
                //数据内容只能被储存到私有的cache，仅对某个用户有效，不能共享
        ...
        if (constraints != null) {
            ...
            Debug.log(" Calling hasUserDataPermission()",module);
            if (!realm.hasUserDataPermission(request, response, constraints)) {
                Debug.log("Failed hasUserDataPermission() test",module);
                /**
                 * 注:这是在没有权限的情况下执行的
                 * 认证器已经设置了相应的状态码，所以在这里什么也不做直接返回
                 * */
                return;
            }
        }
        ...
        if (!authRequired && context.getPreemptiveAuthentication()) {
            authRequired = request.getCoyoteRequest().getMimeHeaders().getValue(
                        "authorization") != null;
        }
        ...
        if(authRequired) {
            Debug.log("Calling authenticate()",module);
            if (!authenticate(request, response)) {
                Debug.log("Failed authenticate() test",module);
                return;
            }
        ...
        if (constraints != null) {
            Debug.log("Calling accessControl()",module);
            if (!realm.hasResourcePermission(request, response, constraints, this.context)) {
                Debug.log("Failed accessControl() test",module);
                return;
            }
        }
        Debug.log("Successfully passed all security constraints",module);
        getNext().invoke(request, response);
