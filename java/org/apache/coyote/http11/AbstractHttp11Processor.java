...
public abstract class AbstractHttp11Processor<S> extends AbstractProcessor<S> {
    ...
    public static final String module = AbstractHttp11Processor.class.getName();
     ...
     * If the encoding is not supported, a 501 response will be returned to the client.
     */
    ...
    public final void action(ActionCode actionCode, Object param) {
        ...
        case COMMIT: {
            ...
            /**
             * 记录这个header数据是否flush出去了
             * 注意，如果之前flush了一次，那就证明了header已经全部flush出去了，后面在有header数据也不flush了
             * 因为之前flush header之后，已经flush 响应数据了，再flush header就会出错（不是http响应了）
             * */
            try {
                /**先生成header数据，然后放到coyoteResponse的buffer中*/
                prepareResponse();
                /**把上面buffer中生成的数据commit出去*/
        ...
        case ACK: {
            //如果已经提交头部数据或者不需要发送大数据就不需要想客户端发送消息
            if ((response.isCommitted()) || !expectation) {
                return;
            }
            Exception exception = new Exception("发送大数据");
            Debug.logError(exception,"请检查",module);
     ...
     * This method allows the processor implementation to determine if keep-alive should be disabled or not.
     */
        ...
        while (!getErrorState().isError() && keepAlive && !comet && !isAsync() && upgradeToken == null && !endpoint.isPaused()) {
            ...
            try {
                ...
                if (endpoint.isPaused()) {
                    //503是一种HTTP状态码。英文名为503 Service Unavailable
                ...
                Debug.log(e,sm.getString("http11processor.header.parse"),module);
                setErrorState(ErrorState.CLOSE_NOW, e);
                break;
            } catch (Throwable t) {
    ...
    protected void parseHost(MessageBytes valueMB) {
        ...
        //如果valueMB为空 即没有host,将客户端socket
