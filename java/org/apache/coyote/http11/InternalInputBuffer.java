...
public class InternalInputBuffer extends AbstractInputBuffer<Socket> {
    ...
    public boolean parseHeaders()
        ...
        if (!parsingHeader) {
            ...
            throw new IllegalStateException("Unexpected state: headers already parsed. Buffer not recycled?");
        }
     ...
     * 这个方法是以行进行解析
     * k:v
     * MimeHeaderField实例nameB存放k valueB存放value
     * */
    @SuppressWarnings("null") // headerValue cannot be null
    private boolean parseHeader()
            throws IOException {
        //检查换行 将pos移到第一个有效字符 在这之前已经解析了 方法名 uri 协议
        byte chr = 0;
        ...
        while (true) {
            ...
            if (chr == Constants.CR) {
                 ...
                 * 因为在上一次key：value中已经解析了换行符了，如果再次开头遇到那么就表示解析完了
                 * 因为两次同时遇到换行符只能是消息体了
                 */
                pos++;
                return false;
        ...
        //标记buf当前位置，这样是为了记录pos的位置，由于其会自增
        int start = pos;
        boolean colon = false;
        MessageBytes headerValue = null;
        //这个循环是读读取key值 并创建MimeHeaderField对象到MimeHeaders实例的MimeHeaderField数组句柄
        while (!colon) {
            if (pos >= lastValid) {
                if (!fill())
                    throw new EOFException("Unexpected EOF read on the socket");
            }
            //一直循环找到： 将前面的字符串解析 创建一个MimeHeaderField字段 将name传进去
        ...
        start = pos;
        //FIXME:这只是为了查看
        char[] charArray = CharUtil.getChars(buf);
        ...
        while (validLine) {
            ...
            /**
             * buf[realPos] = buf[pos];的作用是填充上文去掉空格的部分
             * lastSignificantChar的作用是定位到最后一个不是空格的位置索引
             * */
            ...
            while (!eol) {
                ...
                //如果是\r什么也不做
                if (buf[pos] == Constants.CR) {
                    //如果是\n则eol设为true 表示已经读取完此行的key:value了
                } else if (buf[pos] == Constants.LF) {
                    eol = true;

                    //如果是空格赋值给 buf[realPos]
            ...
            chr = buf[pos];
            //如果不是空格或者制表符则validLine设置为false,要不然说明后面还有内容
            //FIXME:这段代码暂时未理解
