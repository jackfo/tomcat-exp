...
public class BaseModelMBean implements DynamicMBean, MBeanRegistration, ModelMBeanNotificationBroadcaster {
     ...
     * 为当前Mbean指定的属性注入值
     *
     * @param attribute 设置属性名和其新值
     *
     * @exception AttributeNotFoundException 如果属性不被这个Mbean支持
     * @exception MBeanException 如果对象初始化扔出异常
     * @exception ReflectionException 在调用getter方法的时候出现反射出现异常
     */
    @Override
    public void setAttribute(Attribute attribute) throws AttributeNotFoundException, MBeanException, ReflectionException
    {
        ...
        try {
            //检车这个方法所在的类是否与当前实例类相同或是当前实例的超类或接口 如果是调用当前实例的方法 反之调用资源类的方法
