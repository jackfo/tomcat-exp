...
public class Nio2Endpoint extends AbstractEndpoint<Nio2Channel> {
    ...
    public void bind() throws Exception {
        ...
        //设置套接字缓冲区接受的大小，一个端口释放后会等待两分钟之后才能再被使用，SO_REUSEADDR是让端口释放后立即就可以被再次使用
        ...
        try {
            ...
            if (channel == null) {
                ...
                if (sslContext != null) {
                    ...
                    //创建一个NioBufferHandler 缓冲区 其中读写的属性值就是socketProperties中对应的
                    //getDirectBuffer决定是使用本地缓存还是jvm缓存区
            ...
            while (running) {
                ...
                try {
                    ...
                    try {
                        ...
                        // socket
                        //accept的返回值是Future<AsynchronousSocketChannel> 通过调用get获取AsynchronousSocketChannel
                        //所以这里主要是通过Future模式获取异步socket通道
                        socket = serverSock.accept().get();
                    } catch (Exception e) {
                        //连接数-1
                        countDownConnection();
                        //在异常状态，如果还是运行状态，对异常进行处理并扔出
                        if (running) {
    ...
    public void awaitBytes(SocketWrapper<Nio2Channel> socket) {
        ...
        //获取Nio2Channel  获取缓冲区 并进行读取
        ByteBuffer byteBuffer = socket.getSocket().getBufHandler().getReadBuffer();
        //清除缓冲区
        byteBuffer.clear();
        //获取获取Nio2Channel通道 然后对CompletionHandler<Integer, SocketWrapper<Nio2Channel>> awaitBytes 进行读取
        //执行其中的processSocket0方法
