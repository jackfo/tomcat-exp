...
public final class MessageBytes implements Cloneable, Serializable {
    ...
    public String toString() {
        ...
        //根据类型决定是转化charC还是byteC得值到字符串，如果类型不是T_CHARS也不是T_BYTES则直接返回
        switch (type) {
        case T_CHARS:
    ...
    /**
     * 将MessageByte类型转化为T_CHARS
     * */
    public void toChars() {
        //入过charC不为空则将类型直接设为T_CHARS
        if( ! charC.isNull() ) {
            type=T_CHARS;
            return;
        }
        //如果字符串的值存在类型赋值为char然后将字符串赋为char将其注入charC这个CharChunk
        toString();
        type=T_CHARS;
