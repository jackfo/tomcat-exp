...
public final class CharChunk implements Cloneable, Serializable, CharSequence {
    ...
    public static final String module =CharChunk.class.getName();
     ...
     *  如果是-1或者没有设置 buffer会无限的增长
     *  可以小于当前buffer大小
     *  当达到limit,buffer会刷新或者扔出异常
     */
    ...
    /**
     * out即OutputBuffer对象，在创建OutputBuffer实例的时候，会创建CharChunk实例，然后将OutputBuffer给封装进去
     * */
    ...
    public boolean startsWith(String s) {
        ...
        //获取字符串的长度
        int len = s.length();
        //如果字符串的长度大于buff在当前实例中应有的数据长度则直接返回fasle 如：在匹配Context 是该字符串是Context.name 如果uri还没有其长,那么肯定直接返回false
        if (c == null || len > end-start) {
            return false;
        }
        //从开始遍历到context.name的长度 在其长度返回内完全相等返回真
        int off = start;
        for (int i = 0; i < len; i++) {
            if (c[off++] != s.charAt(i)) {
                return false;
